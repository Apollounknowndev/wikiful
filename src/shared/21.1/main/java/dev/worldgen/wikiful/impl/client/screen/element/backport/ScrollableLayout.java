...
public class ScrollableLayout implements Layout {
	...
	class Container extends WikifulContainerWidget {
		...
		public Container(final Minecraft $$0, final int $$1, final int $$2) {
		...
		public void setFocused(@Nullable GuiEventListener $$0) {
			...
			if ($$0 != null && this.minecraft.getLastInputType().isKeyboard()) {
				...
				if ($$3 < 0) {
					this.setScrollAmount(this.scrollAmount() + (double)$$3 - (double)14.0F);
				} else if ($$4 > 0) {
					this.setScrollAmount(this.scrollAmount() + (double)$$4 + (double)14.0F);
				}
